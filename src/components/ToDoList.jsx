...
function ToDoList() {
  ...
  function handleChecked(index){
    if (tasksRef.current[index].classList.contains('line-through')) {
      tasksRef.current[index].classList.remove('line-through')
    } else {
      tasksRef.current[index].classList.add('line-through')
    }
  }
      ...
      <div className="overflow-y-scroll flex flex-col justify-start items-center gap-2 w-full h-[80%]">
        {data?.map((item, index) => (
          <div key={index} className="flex flex-row justify-between items-center px-5 py-2 w-full hover:bg-gray-200 rounded-3xl">
            <div className="flex flex-row gap-5 items-center" ref={(el)=>tasksRef.current[index] = el}>
              <input {...register('checked')} onChange={()=>handleChecked(index)} id={`task-${index}`} type="checkbox" className="size-[18px]" />
      ...
      {editingIndex !== null && (
        <div className="size-full bg-[rgba(0,0,0,0.5)] flex flex-col justify-center items-center absolute rounded-2xl">
          <form id="edit" className="bg-white rounded-2xl px-5 py-5 w-[30svw] h-[20svw] flex flex-col justify-center items-center gap-8" onSubmit={handleSubmit(submit)}>
            <label htmlFor="editTask" className="font-bold text-2xl">Edit Task</label>
